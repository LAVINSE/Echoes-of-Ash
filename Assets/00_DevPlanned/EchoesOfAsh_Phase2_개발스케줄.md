...
> 기준 문서: 기획서 v3.2 (16장 Phase 2 체크리스트) + Phase 1 개발스케줄 개정 11 · 작성일: 2026-07-22 (개정 24 — 2026-07-29, P2-M7 7-5 카드 해금 2종 완료: 해금 원장 = Town 구획 + 즉시 저장)
...
| 7-4 | 이벤트 10개 / 상점 / 보상 화면 | 데이터 기반 선택지·구매·카드 보상 — **던전 구성 데이터(챕터 SO) 신설 ✅ 완료 (개정 21 — 착수 기록 참조. 노드 이벤트 매핑 흡수(개정 11 결정) + `statusDatas` 흡수(개정 13 결정) + `sanityEventDatas` 흡수(P2-M0 결정))**. 상점·보상 화면 = **뷰 코드째 아트 시점 이월 (개정 23 — 스타일 확정 후 코드 작업)**, 획득/구매 로직 계층은 선행 가능. 이벤트 10개 = 리소스 작업 |
| 7-5 | 카드 해금 2종 | 발견형 자동 해금 + 제작형 설계도 해금 (메타 저장 연동) — **✅ 완료 (개정 24 — 완료 기록 참조. `CardUnlockService` + 해금 원장 = Town 구획 + 설계도 매핑 = ItemData + 기본 해금 합집합)**. 발견형 등장 확률·굴림 = 7-4 보상 로직 잔여 |
