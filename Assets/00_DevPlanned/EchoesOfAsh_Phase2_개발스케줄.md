...
> 기준 문서: 기획서 v3.2 (16장 Phase 2 체크리스트) + Phase 1 개발스케줄 개정 11 · 작성일: 2026-07-22 (개정 29 — 2026-07-30, 아트 게이트 착수 — 아트 작업 계획서 신설·리소스 작업보다 선행 확정)
...
- **아트 부담 억제 유지**: Phase 2도 플레이스홀더 — 기능적 아트는 리소스 작업 시점 → **UI 작업 방침 개정 (개정 23 — 사용자 결정): 신규 전용 뷰(보상 화면·상점·마을 팝업·보관 선택 UI 등)는 플레이스홀더 선작성 없이 아트 시점에 스타일을 먼저 잡고 그에 맞춰 뷰 코드 작업.** 기존 플레이스홀더 뷰는 유지 (교체 시 유지 계약 = Init/Release·구독+표시만 — 개정 17). 뷰 뒤의 로직 계층(굴림·자원·구매 API 등)은 선행 가능
...
| 7-2 | 유물 20개 | 트리거 구조(P2-D4) + 정신력 연동 포함 — **유물 시스템 ✅ 완료 (개정 22 — 완료 기록 참조. RelicData + 획득 관리 + 획득 순 발화 계약 + 미배선 트리거 3종 연결)**. 콘텐츠 20개 제작은 잔여 (리소스 작업) |
| 7-3 | 적 12종 + 조우 테이블 | SAN 압박 행동 포함 — **`SpawnRange` 조우 풀 실가동 ✅ 완료 (개정 26 — 완료 기록 참조. 층 필터 = 구성 적 SpawnRange 결합)**. 적 12종(일반 9 + 엘리트 3) + 조우 14종 구성안 = **데이터 문서 정리 완료** — 에셋 제작·실검증 = **아트 작업 시점 (사용자 결정)** |
| 7-4 | 이벤트 10개 / 상점 / 보상 화면 | 데이터 기반 선택지·구매·카드 보상 — 던전 구성 데이터(챕터 SO) 신설 ✅ 완료 (개정 21) · **로직 계층(보상 굴림·골드 자원·구매/카드 제거 API + 몬스터 드랍형 카드) ✅ 완료 (개정 25 — 완료 기록 참조)**. 잔여 = 상점·보상 화면 뷰 (**아트 시점 — 개정 23**) + 이벤트 10개 (리소스 작업) |
| 7-5 | 카드 해금 2종 | 발견형 자동 해금 + 제작형 설계도 해금 (메타 저장 연동) — **✅ 완료 (개정 24 — 완료 기록 참조. `CardUnlockService` + 해금 원장 = Town 구획 + 설계도 매핑 = ItemData + 기본 해금 합집합)**. 발견형 등장 확률·굴림 = 7-4 보상 로직 잔여 → ✅ 이행 (개정 25 — `RollBattleReward` 발견형 등장 판정) |
| 7-6 | 보스 1개 | HP 페이즈 패턴 활용 (구조는 M4에서 기완성) → **✅ 구성안 완료 (개정 27 — 완료 기록 참조. 데이터 문서 2-3절 편입, 에셋 제작·실검증 = 아트 작업 시점)** |

#### P2-M7 7-6 완료 기록 — 챕터 1 보스 구성안 (2026-07-30)

- **코드 0줄 확인:** 다단 HP 페이즈(최저 임계값 우선)·광기 패턴·보스 전용 풀(`bossEncounters` — 개정 21)·보스 골드 구간(`RewardConfigData` — 개정 25)·보스 노드 승리 = 던전 승리(`EndDungeon(true)`) 경로 전부 기존 구조 소비 — 신규 코드 없음
- **구성안 = 데이터 문서 증보 (개정 1):** `Enemy_AshMonarch` "잿더미의 군주" — HP 150 / SAN 120/120/40 / 어그로. 기본 → HP ≤ 60% "타오르는 옥좌" → HP ≤ 25% "붕괴하는 군주" (방어 없는 전면 공세) — **다단 HP 페이즈 최저 임계값 규칙 첫 실가동 대상**
- **광기 = "탈진 락" 설계 (⚠ 사용자 결정 후보):** M4 우선순위 계약(광기 > HP 페이즈)을 그대로 활용 — 정신 공격 투자로 보스를 최종 페이즈 진입 전에 광기에 가둠 (방어·취약 소실 대신 SAN 압박 유지·집중 + SAN 120 = 고투자 요구). **챕터 1 광기 대칭: 전령 = 리스크(강화) / 보스 = 보상(봉인)** — 기획서 3-2 "적마다 다르게" 이행. 대안(전령식 강화 통일)은 데이터 문서 미결정 3번 기록
- **조우·드랍:** `EnemyEncounter_AshMonarch` 단독 — SpawnRange 미설정 (전용 풀이라 층 필터 무관 — 잠정 규칙 ① 재사용). 드랍 = 설계도 위주 + 카드 드랍 슬롯("군주의 잿불" — 7-1 연결)·보스 유물(Unique 매핑 — 개정 22) 예약
- **에셋 제작·실검증 = 아트 작업 시점 (기존 방침)** — 데이터 문서 체크리스트 7·8번·시나리오 증보분 참조. **⚠ 체크: `EEnemyType.Boss` enum 값 존재 여부** — 없으면 기존 값 뒤 추가 (직렬화 안전 — Taunt 전례)
- **P2-M7 잔여 정리:** 코드·구성안 계층 전량 완료. 남은 것 = 리소스 작업(카드 50장·유물 20개·이벤트 10개 — 7-1·7-2·7-4 잔여)과 아트 시점 뷰/에셋/실검증뿐

#### P2-M7 7-3 완료 기록 — SpawnRange 조우 풀 실가동 + 적/조우 구성안 (2026-07-30)

- **코드 (프로젝트 반영 확인):** `DungeonChapterData.GetRandomEncounter(nodeType, floor)` — 층 필터 확장 (Phase 1 개정 4 "SpawnRange 결합 조우 풀" 예고 이행). **등장 구간 소유 = `EnemyData.SpawnRange` 재사용** (조우 필드 무증축·데이터 중복 없음), 조우 등장 가능 = 구성 적 전원의 구간이 층 포함 (`EnemyEncounterData.IsSpawnableAtFloor`). `StartBattleForNode` 층 전달
- **잠정 규칙:** ① `(0, 0)` = 미설정 = 무제한 (기존 에셋 무수정 호환 — 0층 전용 표현은 불가, 실요구 없음) ② 층 = **0-based** (입구 = 0 — 잿불 침식 "잠식 층 0~" 표기 일치) ③ **필터 공집합 = 필터 무시 폴백 + 경고** (조우 유실은 런 진행을 막지 않음 — 빈 전용 풀 폴백 전례)
- **개정 25 반영 대기분 해소 확인:** `EnemyEncounterData` 카드 드랍 가중치판 (`noCardDropWeight` + `CardDropEntry` 목록 + `RollDropCard`) + `RollEncounterCardDrop` 간소화 — 프로젝트 반영 확인
- **콘텐츠 구성안 = `EchoesOfAsh_데이터문서.md` 신설 (사용자 결정 — 별도 문서 정리):** 적 12종 (일반 9 + 엘리트 3 — 기획서 14-5 목업 3종(재의 망령·정신 포식자·잿빛 골렘) 정식 편입) + 조우 14종 (일반 10 + 엘리트 4 — 등장 구간 = SpawnRange 자동 결합). 행동 = **기존 효과 블록만** (피해/방어막/상태이상/정신력 — 코드 0줄 검증), 광기 패턴 = 전령 1종만 (콘텐츠 비용 제어)·HP 페이즈 = 골렘 (실가동 검증)·대상 규칙 3종 전부 실전 배치 (어그로 = 사냥개·지정 = 포식자/파수꾼), 몬스터 드랍형 카드 슬롯 2건 예약 (7-1 연결). **전 수치 잠정 (밸런스 영역 — 파티 3인 기준)**
- **에셋 제작·실검증 = 아트 작업 시점 (사용자 결정)** — 데이터 문서의 체크리스트·시나리오 참조
- **미결정 (데이터 문서 6장):** ① **광기 보상 공통 이득** (기획서 3-2 임시 제안 — 광기 패턴 없는 적에 대한 정신공격 가치 보장. 구현 시 StatusDamageCalculator 래핑 + Balance 외부화, 채택 = 프로토타입 검증 후) ② `HeraldEscort` 조우의 신도 구간 상향 여부

#### P2-M7 7-4 완료 기록 — 로직 계층 + 몬스터 드랍형 카드 (2026-07-30)

- **산출물 (뷰 제외 = 개정 23 방침):** `Data/RewardConfigData` (골드 범위 3종(일반/엘리트/보스) + 카드 보상 굴림 — 선택지 3·등급 가중치(일반 70/희귀 25/에픽 5 잠정)·발견형 등장 확률 0.1 잠정. 굴림 = SO 소유(DropTableData 전례)·**풀 주입 방식 = Data → Save 의존 차단**, 해금 확정 = 조립 지점 소관 — **원장/굴림/소비처 3분리**) + `Data/ShopConfigData` (슬롯·등급별 가격·제거 비용 — 기획서 12-2 잠정치, 에픽 구간 잠정 신설) + `Dungeon/ShopService` (순수 클래스 — 재고 굴림·구매 2종·카드 제거 상점당 1회·실패 = 환불, 저장 시점 = DungeonManager 소관)
- **골드 = DungeonState 소유** (던전 1회 수명 — 유물 전례. `AddGold`/`TrySpendGold` = 검사 후 일괄 차감 계약 계열) + `DungeonSaveData.gold` (**필드 추가·버전 유지** — 기본 0 = 구저장 호환) + `ResumeDungeon` 복원. **잠정: 런 종료 시 소멸 (STS식)** — 마을 회수 규칙 = 미결정
- **보상 파이프라인 확정:** 승리 확정 직후·보스 분기 앞 — ① 조우 아이템 드랍 → ② 조우 카드 드랍 → ③ 골드·카드 보상 (**호출 순서 = 판정 순서**, 가산 관계·중복 지급 없음). 발견형 등장 = 즉시 영구 해금 + `pendingRewardCards` 공개 (보상 화면 표시 원본 대비). 카드 선택 = `SelectBattleRewardCard`/`SkipBattleRewardCard` 공개 API
- **해금 방식 3원화 (사용자 결정): `ECardUnlockType.EnemyDrop`** (끝 추가 = 에셋 무영향) — 특정 조우 한정 드랍 카드, 도감 발견 = 기존 원장 재사용 (원장 = 사실만 기록·스키마 무확장), `TryUnlockByEnemyDrop` + `IsPoolEligible` 제외 = **보상/상점/발견형 3경로 동시 차단 (단일 관문)**, 설계도 경로 = 기존 타입 검사 자동 차단 (코드 0줄)
- **드랍 소유 = EnemyEncounterData (사용자 결정** — 적 = 조우 단위 소환, 적 개체 소유 기각): 조우가 아이템/카드 드랍을 나란히 소유 = **드랍 소유 단위 통일**. **`EnemyData.dropTableId` 삭제 (사용자 결정 — 미사용 잔재, 대체 없음)**
- **카드 드랍 = 가중치 추첨 개정 (사용자 결정):** 최초 반영분(단일 카드 + 확률) → **`noCardDropWeight` + `CardDropEntry` 가중치 후보 목록** (DropTableData 미러 — 꽝 가중치·부동소수 보정·순회 순서 = 판정 순서, 카드 = 수량 없음이라 가중치만). `RollDropCard` = SO 소유, 조우당 1회 굴림·최대 1장 (잠정 — 다중 드랍 요구 시 굴림 횟수 확장). ⚠ 가중치판 = **프로젝트 반영 대기** (`EnemyEncounterData` 전문 교체 + `RollEncounterCardDrop` 간소화)
- **상점 잠정 규칙:** 발견형 등장 = 전투 보상 한정 (상점 = 해금 풀만) / 유물 재고 = 미보유만 / 미해결 노드 재진입 = 재고 재굴림 (스키마 무확장) / 소모품 슬롯 = 포션 시스템 등장 시 증축 (1-5) / 전설·고유 = 보상·상점 풀 제외 (확장 예약분) / 미선택 카드 보상 = 스냅샷 미편입 (재로드 = 소멸 — 보상 화면 도입 시 재검토)
- **프로젝트 반영 확인 (개정 25):** `RewardConfigData`·`ShopConfigData`·`ShopService`·`CardUnlockService`(EnemyDrop)·`GameEnum`·`DungeonState`(골드)·`DungeonSaveData/Service`(gold 기록)·`DungeonManager`(파이프라인·Shop 분기·복원·**EndDungeon 정리(shopService·pendingRewardCards) 확인**·테스트 버튼 5종)·`EnemyEncounterData`(단일 카드판) 전부 반영 확인
- **임시 조치:** DungeonManager 테스트 버튼 5종 (골드 지급·보상 선택·상점 카드/유물 구매·카드 제거 — 보상/상점 뷰 도입 시 제거) + `testRelicData`(개정 22 지속)
- **사소 잔여 ⚠ (다음 코드 작업 시):** DungeonManager `GetNodeEventData`~`OpenShop` 사이 빈 줄 2개 정리
- **테스트 방침 (사용자 결정):** 실검증 전반 = **아트 작업 시점 일괄** — 아트 적용 상태가 직관적 (개정 22·24 이월분과 함께)
- **미결정 (다음 작업 전 확인):** ① 골드 런 종료 처리 (소멸 vs 일부 마을 회수) ② 보스 승리 = 즉시 런 종료라 보스 보상 실효 없음 (다챕터/결과 화면 설계 시 결정) ③ 상점 재고 등급 분포 (균등 vs 슬롯별 등급 고정)

#### P2-M7 7-5 완료 기록 — 카드 해금 2종 (2026-07-29)

- **산출물:** `Save/CardUnlockService.cs` — 카드 해금 원장의 정적 파사드 (TownSaveService 구획 파사드 전례). 해금 상태 판정(`IsUnlocked`) + 풀 후보 조회(`CollectUnlockedCards`/`CollectDiscoveryCandidates` — 결과 목록 파라미터 패턴, GetAvailableNodes 전례) + 해금 2경로(`TryUnlockByDiscovery`/`TryUnlockByBlueprint`)만 담당. **추첨(등장 확률·굴림)은 보상/상점 로직 소관 (7-4)** — S 원칙: 원장과 소비처 분리
- **해금 원장 위치 = Town 구획:** `TownSaveData.unlockedCardCodeNames` (**필드 추가·버전 유지** — 개정 19 규칙, relicCodeNames·ownedCharacterCodeNames 전례). 발견형/제작형 공용 단일 목록 — 해금 방식 구분은 CardData.unlockType이 소유, 원장은 사실(해금됨)만 기록. **해금 확정 = 즉시 파일 저장** (기획서 13-1 "발견형 즉시 영구 확정" — 보관 전송 즉시 귀속 전례와 동일 원리: 전송/해금 순간 기록 = 이후 사망 무관 보존 자연 성립)
- **설계도 → 카드 매핑 = `ItemData.unlockCard` 필드:** 설계도 아이템 자체가 해금 대상을 식별 (Dead Cells식). 별도 매핑 SO·서고 레시피 구조 기각 — 설계도 = 카드당 1에셋이라 아이템 자체 식별이 관리 최저 비용 (1-5 원칙 — 실요구 전 구조 확장 금지). OnValidate: 타입-필드 불일치 경고 2건 (BluePrint인데 카드 미연결 / 그 외 타입인데 카드 연결)
- **기본 해금 = `CardData.isDefaultUnlocked` + 조회 시 합집합:** 첫 런 보상 풀 공백 방지. 저장 파일 시딩 없이 **조회 시점 `기본 해금 ∪ 원장` 합집합** — 시딩·마이그레이션 이슈 원천 차단, 초기 풀 구성 = 카드 에셋 체크만 (코드 0줄 확장). 기본 해금 카드는 원장에 기록하지 않음
- **잠정 규칙:** ① 풀 조회 제외 = **강화 카드(IsUpgrade)·저주(Curse)** — 강화 = 휴식/수련 경로, 저주 = 적이 심는 카드라 풀 등장 자체가 없음 ② 중복 해금 = 무시 (유물 전례) ③ **이미 해금된 카드의 설계도 = 미소모 + 경고** (자원 증발 방지) ④ 설계도 소모 = 검사 후 일괄 차감 (`TryConsumeItems` 재사용 — 부분 차감 방지 계약 승계) ⑤ 제작형 카드는 발견형 경로로 해금 불가 (경고) — 이원화 경계 코드 강제
- **임시 조치:** TownManager `testBlueprintItem`(제거 = 봉인된 서고 팝업 도입 시)·`testDiscoveryCard`(제거 = 7-4 보상 굴림 도입 시) + 테스트 버튼 2종 — 제거 시점이 달라 필드별 Tooltip에 명기
- **프로젝트 반영 확인 (개정 24):** `CardUnlockService` 신설·`TownSaveData` 필드·`CardData.isDefaultUnlocked`·`ItemData.unlockCard`+OnValidate·TownManager 테스트 2종 전부 반영 확인
- **사소 잔여 ⚠ (다음 코드 작업 시):** TownManager 신규 테스트 버튼 2개가 region 밖 클래스 말미 배치 — 기존 `#region 임시 테스트` 구획으로 이동 (전 멤버 region 소속 스타일 정합)
- **에디터 작업 체크리스트:** ① 제작형 카드 1장 지정 (기존 카드 `unlockType = Blueprint` 전환 또는 테스트 카드 신설) ② `Item_Blueprint_Test.unlockCard` 연결 ③ 시작 덱 카드 3장 `isDefaultUnlocked` 체크 (초기 해금 풀 확보) ④ TownManager 프리팹 `testBlueprintItem`·`testDiscoveryCard` 연결 (데이터 윈도우 증축 불필요 — Card·Item 탭 기존 관리 범위)
- **실검증 이월 ⚠ (사용자 결정 — 아트 작업 시점에 일괄):** 설계도 드랍 획득 → 마을 귀환 → 테스트 설계도 해금 → `save.json` `unlockedCardCodeNames` 기록 + 설계도 수량 차감 → 재실행 = "이미 해금" 경고 + 미소모 → 테스트 발견형 해금 → 프로필 재로드 후 해금 유지 → 풀 조회에 강화/저주 미포함 (7-4 보상 로직 검증과 겸행)

#### P2-M7 7-2 완료 기록 — 유물 시스템 (2026-07-29)

- **산출물:** `Data/RelicData.cs` — **Data 흡수** (개정 13 전례: 데이터 윈도우 관리 타입 = Data 네임스페이스). **`05_Scripts/Relic/` 폴더 신설 보류** — 신설 기준: 유물 고유 런타임 로직(커스텀 블록 다수·해금 관리 등)이 실제 등장할 때 (1-5 원칙 — 실요구 전 구조 확장 금지). 필드: 등급(`ERarityType`)·전용 캐릭터 참조(null = 공용)·`TriggerEffect` 목록(**D4 패시브 구조 공유** — CharacterData.passives와 동일 타입). 표시명/설명/아이콘/카테고리 = SWIdentifiedObject 재사용, 정신력 연동 = SanityCondition 파생(`IsSanityLinked`)
- **등급 결정 (사용자 결정):** `ERarityType` **무수정** — Boss 값 신설안·Unique → Boss 개명안 모두 기각 (값 무증가·기존 에셋 무영향). **보스 유물 = 기존 `Unique(고유)` 값 매핑** — 기획 용어 "보스" ↔ 코드 "고유" 분리 매핑 (Madness = 구간 / SanityEvent = 이벤트 전례)
- **획득 관리 = DungeonState 소유 (유물 = 던전 1회 수명, STS식):** `relics` 목록 (**획득 순 유지 = 발화 순 계약의 원본** — 기획서 15-2) + `AddRelic`(중복 = 경고 무시 — **유물 유일 보유 잠정 규칙**, STS 표준) / `HasRelic`. 메타 해금(제련소)은 마을 소관 — 이번 범위 아님
- **저장 편입:** `DungeonSaveData.relicCodeNames` (**필드 추가·버전 유지** — 기본 빈 목록 = 구저장 호환, Town 전례. 목록 순서 = 획득 순 보존). 복원 = `relicDatabase` 코드명 조회, **미등록 코드명 = 경고 스킵** (유물 유실은 런 진행을 막지 않음 — 아이템 전례, 덱이 비면 런 불능인 카드의 실패 처리와 구분). **임시 조치:** DungeonManager `testRelicData` + "테스트 유물 획득" SWButton — **보상 화면(7-4 잔여) 도입 시 제거**
- **발화 순서 계약:** 전투 시작 등록 = **파티 패시브(스폰 순) → 유물(획득 순)** — 단일 디스패처 등록 순 = 발화 순 (D4 구조적 보장)
- **개정 16 예고 의존 정리 이행:** `TriggerEffectController` 소유자 `CharacterEntity → ITargetable` 전환 + `HandleCardPlayed(CardInstance, ESanityType)` 제거 → BattleManager 어댑터 `HandleCardPlayedForTrigger` 중계 = **Effect.Trigger의 Battle·Card 참조 소멸** (구독/해제 대칭 유지)
- **파티 범위 등록 (공용 유물):** `Register(효과 목록, Func<ITargetable> 시전자 공급자, 출처명)` 오버로드 — 시전자·효과 대상 = **발화 시점 첫 생존자** (SanityEventRunner·GetCasterFor 폴백 전례, 잠정 규칙). 전용 유물 = 소유 캐릭터를 소유자로 등록 (**사망 시 미발화** — 전용 카드 드로우 제외와 대칭), 소유자 파티 부재 = 미등록 + 경고. 피격자 대상형 유물("맞은 사람이 방어막")은 커스텀 블록 폴백 영역 (잠정)
- **미배선 트리거 3종 발화 지점 연결 (개정 16 등록 시 경고 블록 제거):**
  - **TakeDamage** = 파티원 `OnDamaged` 구독 → `RaiseFor(피격자)` — 원본 피해 > 0이면 발화 (**방어막 전량 흡수도 피격 인정** — 잠정). 당사자 귀속 발화 = 본인 소유 효과 + 파티 범위 효과만
  - **DealDamage** = 적 `OnDamaged` 구독 + **카드 귀속 구간** — `PlayCard` try/finally에서 `dealDamageAttributionCaster` 설정 (어그로 귀속과 병행·동일 개념). **귀속 밖 피해(상태이상 틱 등) = 미발화** (어그로 잠정 규칙 일치)
  - **BattleEnd** = `EndBattle` 내 **승리 시에만 1회** (잠정 규칙 — 패배 = 던전 종료라 무의미, STS "승리 시 회복" 아키타입. 8-1 재검토 종결). 발화 위치 = `turnManager.EndBattle()` 직후·**덱 리셋/뷰 해제 이전** — 효과의 SAN 변경이 `SetCarriedSanity` 이월에 반영
- **재진입 가드 (잠정 규칙):** `isRaising` 플래그 — **발화 중 재발화 = 무시** (가해 트리거 효과가 다시 피해를 만드는 무한 연쇄 방지 — 개정 21 "효과 파이프라인 중첩 위험" 계열 방어). 연쇄 발동 유물이 콘텐츠로 필요해지면 큐잉 방식 재검토
- **해제 대칭:** `ResetBattle` — `damageTriggerHandlers` 구독 전량 해제 + `dealDamageAttributionCaster` null + 어댑터 구독 해제
- **데이터 윈도우 유물 탭 증축:** 배열 인덱스 정합 ("유물"·`Assets/02_Res/Data/Relic`·`Relic_`) — `Relic_` 기본 에셋 생성으로 탭 동작 확인
- **사소 정리 소화 (개정 18·21 잔여):** ✅ `EEnemyType.Noraml → Normal` ✅ `TargetResolver` 로그 태그 `[TargetResolver]` 통일 ✅ `CardDragController` 미사용 `isSingleTarget` 필드 제거
- **프로젝트 반영 확인 (개정 22):** `RelicData`·`TriggerEffectController` 전문 개정(등록 2형·RaiseFor·재진입 가드)·`DungeonState`(relics)·`DungeonSaveData/Service`(relicCodeNames 기록/복원)·`DungeonManager`(relicDatabase·테스트 버튼)·`BattleManager`(등록 순서·어댑터·피격/가해 배선·귀속·BattleEnd·해제 대칭·`FindPartyMember`)·`GameEnum`(Normal 수정·ERarityType 무수정) 전부 반영 확인
- **사소 잔여 ⚠ (다음 코드 작업 시):** ① `RelicData` 등급 Tooltip에 "보스 유물 = 고유 등급" 매핑 문구 추가 (오배정 방지) ② `BattleManager.statusDatas` Tooltip "(임시 조치)" → "(BattleTest 폴백)" (개정 21 잔여 지속) ③ `EnemyAI` 생성자 주석 `party`·`aggroSystem` 파라미터 설명 (개정 18 잔여 지속) ④ `BattleBalanceData.sanityEvent` 잔재 확인 후 제거 (개정 21 잔여 — 미확인) ⑤ DungeonManager 낡은 Tooltip 3건 — `characterDatas` "(4-4로 대체 예정)"·`availableCharacters` "(메타 저장 시 대체)" = 이미 해소된 임시 조치 문구 잔존 / `itemDatabase` "(저장 스키마 v3)" = 버전 1 리셋 전 표기
- **실검증 이월 ⚠ (사용자 결정 — 아트 작업 시점에 아트 추가와 함께 일괄):** 테스트 유물 획득 → 다음 전투 발화 확인 → 평정/광기 조건 분기 → 이어하기 후 유물 보존 + 획득 순 유지 → 피격/가해/전투 종료 트리거 유물 각 1종으로 3종 발화 → 전용 유물 소유자 사망 시 미발화 → 재진입 가드(가해 트리거에 피해 블록 = 1회만 발화) → 중복 획득 무시 확인
...
### P2-M8 — 통합 검증 (1주) — 코드 경로 검증 완료 (개정 28), 실플레이 = 아트 작업 시점

**DoD (Phase 2 전체):** 거점 → 파티 편성 → 런 1회 완주(보스 처치 or 게임 오버) → 회수 → 거점 복귀 → 해금 반영이 저장/복원 포함 완주된다. → **코드 경로 전 구간 성립 확인 (개정 28 — 완료 기록 참조), 실플레이 완주 = 아트 작업 시점 이월분과 일괄.**

#### P2-M8 완료 기록 — 통합 검증 코드 경로 (2026-07-30)

- **검증 방식:** DoD 전 구간 코드 추적 — 상태 전이(런치 요청·화면 상태·저장 3시점 계약)·데이터 흐름(덱/골드/유물/소지/해금)·종료 경로 3종(보스 승리·전투 패배·침식 패배) 점검. 6구간 전부 경로 성립 확인 (구간별 결과 = `EchoesOfAsh_P2-M8_통합검증_보고.md`)
- **발견 이슈 ① 보관 전송 이중 지급 (실버그·수정 ⚠ 프로젝트 반영 대기):** `TransferCarriedToStorage`가 Town 귀속 후 `TownSaveService.Save()`만 호출 — 파일의 dungeon 구획은 진입 직전 스냅샷(소지 잔존) 그대로. 노드 화면 완료 전 종료 → 미해결 재실행 = 재전송 (무한 복제 세이브스커밍). **수정 = 원자화:** 소거 후 `SaveDungeon()` 1회 (통합 저장 P2-D7 = 루트 단일 파일이라 Town 변경분 동반 기록 — Town 단독 저장 호출 제거). 대칭 확인: 회수 경로는 `DeleteSave` 선행 계약이 자연 방어 — 이 버그는 "던전 상태 변경 + Town 단독 저장" 조합 유일
- **발견 이슈 ② `ResumeDungeon` `itemDatabase` null 미가드 (방어 누락·수정 ⚠ 프로젝트 반영 대기):** 소지 복원 NRE 가능 — 유물 전례(null 허용·경고 스킵 — 유실은 런 진행을 막지 않음)로 통일
- **발견 이슈 ③ 상점 잠정 규칙 사문화 → B안 확정 (사용자 결정):** "미해결 재진입 = 재고 재굴림" 규칙 vs 코드 = 진입 즉시 해결 — 재진입 자체가 불가. **개정 25 잠정 규칙 재정의: 재로드 = 미이용 재고 소실 (구매분은 구매 시점 저장으로 보존)** — 코드 무수정, 상점 뷰(아트 시점) 도입 시 A안(미해결 유지 + 종료 지점 해결 처리) 재검토
- **Phase 2 잔여 총정리:** 코드 계층 전량 완료. 잔여 = ① 리소스 작업 (카드 50·유물 20·이벤트 10·적/조우/보스 에셋 — 밸런스 게이트 병행) ② 아트 시점 뷰 (보상 화면·상점·마을 팝업·보관 선택·결과 화면) ③ 실플레이 일괄 검증 (씬 검증 이월분 전체) ④ 미결정 확정 (보스 광기 방향·골드 종료 처리·광기 보상 공통 이득 등)

---

> **◆ 아트 게이트 (사용자 결정 — 밸런스 게이트보다 선행):** 아트 작업 단계의 단일 정리처 = `EchoesOfAsh_아트작업_계획서.md` (초판 — 이월분 전수 통합). 순서 (계획서 개정 1 — **검증 소화 순**, "아트 순서 = 검증 소화 순서" 원칙): **① 한글 폰트 + 공통 UI 킷 (S1·S5·S3 — 최소 비용 즉효) → ② 전투 화면 (S4 카드 규격 + 5-1 잔여·게이지·의도 — 검증 밀집도 최대·전투 아트 = 검증 도구) → ③ 적 스프라이트 + 에셋 제작 (데이터 문서 시나리오 가동) → ④ 맵 화면 (이 시점부터 P2-M8 실플레이 완주 가능) → ⑤ 마을 + 팝업 (V1·V2) → ⑥ 신규 뷰 잔여 (V3 보상 = 최고 가치 → V4~V6) → ⑦ 실검증 잔여 마감.** 밸런스 게이트(6-2~6-5 리소스 제작·수치 튜닝)는 아트 게이트 이후 — 에셋 제작(Step 4)에서 밸런스 게이트분(카드 15·적 3·이벤트 5)과 챕터 1 본편분(적 13·조우 15·보스)을 일괄 소화할지 그 시점에 결정.
...
| 유물 | `RelicData`(정의 SO — **Data 흡수**, 개정 22) + 획득 관리 = **DungeonState 흡수** (획득 순 목록·저장 편입) — 트리거 구조는 패시브 공용 `Effect/Trigger/` (개정 16 — `TriggerEffect`/`TriggerEffectController`). **`Relic/` 폴더 신설 보류 (개정 22)** — 기준: 유물 고유 런타임 로직(커스텀 블록 다수·해금 관리) 등장 시 | `05_Scripts/Data/`, `05_Scripts/Dungeon/`, `05_Scripts/Effect/Trigger/` |
...
| 개정 29 | 2026-07-30 | **아트 게이트 착수 (사용자 결정 — 밸런스 게이트보다 아트 작업 선행)** — `EchoesOfAsh_아트작업_계획서.md` 신설 = 아트 시점 이월분 전수 통합 단일 정리처 (신규 뷰 6종 명세: 소비 API·제거 임시 조치·연동 미결정 / 에디터 체크리스트 13항: 개정 13·18·20·21·24 + 데이터 문서 / 테스트 버튼 제거 맵 / 실검증 10항 / 미결정 8건 누적). **작업 순서 5단계 확정: 스타일 가이드(S1~S5) 선행 → 뷰 6종 (팝업 톤 공유 순: 마을 → 서고 → 보상 → 상점 → 보관 → 결과) → 플레이스홀더 교체 (유지 계약) → 에셋 제작 → 실검증 일괄** (개정 23 방침 이행 — 스타일 미확정 뷰 코드 = 재작업 리스크). **다음: 스타일 가이드 S1~S5 확정 (사용자) → V1 마을 팝업 뷰 코드** |
| 개정 28 | 2026-07-30 | **P2-M8 통합 검증 코드 경로 완료 = Phase 2 코드 계층 전량 완료 (수정 2건 ⚠ 프로젝트 반영 대기)** — DoD 6구간(거점 → 편성 → 런 완주 → 회수 → 복귀 → 해금 + 저장/복원) 전 구간 코드 추적으로 경로 성립 확인 (검증 보고 문서 별첨). **발견 수정 2건 (⚠ 반영 대기):** ① 보관 전송 이중 지급 (Town 단독 저장 → 파일의 던전 구획에 소지 잔존 → 미해결 재실행 = 재전송·무한 복제 — **수정: 소거 후 `SaveDungeon()` 원자화**, P2-D7 단일 파일이라 Town 변경분 동반 기록) ② `ResumeDungeon` `itemDatabase` null 미가드 (소지 복원 NRE — 유물 전례로 통일: 경고 스킵). **상점 규칙 = B안 확정 (사용자 결정):** "미해결 재진입 = 재고 재굴림" 재정의 → 재로드 = 미이용 재고 소실·구매분 보존 (코드 0줄, 상점 뷰 도입 시 A안 재검토). 잔여 = 리소스 작업 + 아트 시점 뷰/실검증 + 미결정 확정. **다음: 밸런스 게이트 (리소스 작업) 또는 아트 작업** |
| 개정 27 | 2026-07-30 | **P2-M7 7-6 보스 구성안 완료 (데이터 문서 개정 1 편입) = P2-M7 코드·구성안 계층 전량 완료** — `Enemy_AshMonarch` "잿더미의 군주" (HP 150·SAN 120·어그로): HP 페이즈 2단 (0.6/0.25 — 최저 임계값 규칙 첫 실가동) + **광기 = "탈진 락"** (M4 우선순위 계약 활용 — 광기가 HP 페이즈를 덮는 기존 구조를 전략 축으로 전환: HP 러시 vs SAN 봉인, 코드 0줄). 챕터 1 광기 대칭 = 전령 리스크 / 보스 보상. `EnemyEncounter_AshMonarch` = bossEncounters 등록안 (SpawnRange 미설정 — 전용 풀 무관)·드랍 = 설계도 위주 + 카드 슬롯("군주의 잿불")·보스 유물 예약. 미결정: 광기 방향 확정(탈진 락 vs 강화 통일 — 사용자)·보스 유물 지급 시점. ⚠ 체크: `EEnemyType.Boss` enum 존재. 에셋 제작·실검증 = 아트 작업 시점. **다음: P2-M8 (통합 검증 — 코드 경로)** |
| 개정 26 | 2026-07-30 | **P2-M7 7-3 SpawnRange 조우 풀 실가동 완료 (프로젝트 반영 확인) + 적/조우 구성안 = 데이터 문서 신설 (사용자 결정)** — `GetRandomEncounter(nodeType, floor)` 층 필터 (Phase 1 개정 4 예고 이행): 등장 구간 = `EnemyData.SpawnRange` 재사용·조우 등장 = 구성 적 전원 결합 (`IsSpawnableAtFloor` — 조우 필드 무증축), 잠정 규칙 = (0,0) 미설정 무제한·층 0-based·**공집합 = 폴백 경고**. 개정 25 반영 대기분(카드 드랍 가중치판) 해소 확인. **`EchoesOfAsh_데이터문서.md` 신설** — 콘텐츠 데이터 단일 정리처 (적 12종: 목업 3종 정식 편입·광기 패턴 = 전령 1종·HP 페이즈 = 골렘·대상 규칙 3종 실전 배치 / 조우 14종: 구간 = 자동 결합 / 행동 = 기존 효과 블록만 — 코드 0줄 검증 / 카드 드랍 슬롯 2건 예약). **에셋 제작·실검증 = 아트 작업 시점 (사용자 결정).** 미결정: 광기 보상 공통 이득·HeraldEscort 구간. **다음: 7-6 (보스 1개 — 구성안은 데이터 문서 증보) 또는 P2-M8 (통합 검증 — 코드 경로)** |
| 개정 25 | 2026-07-30 | **P2-M7 7-4 로직 계층 완료 + 몬스터 드랍형 카드 신설 (프로젝트 반영 확인, 사용자 결정 3건)** — `Data/RewardConfigData`(골드 3구간 + 카드 보상 굴림: 선택지 3·등급 가중치·발견형 확률 — 굴림 = SO 소유·풀 주입 = Data → Save 의존 차단·해금 확정 = 조립 지점: **원장/굴림/소비처 3분리**) + `Data/ShopConfigData`(슬롯·등급별 가격·제거 비용 — 기획서 12-2 잠정치) + `Dungeon/ShopService`(순수 클래스 — 재고 굴림·구매 2종·카드 제거 1회·실패 = 환불, 저장 = DungeonManager). **골드 = DungeonState** (던전 수명·잠정 종료 시 소멸) + `DungeonSaveData.gold` (필드 추가·버전 유지). 보상 파이프라인 = 아이템 드랍 → 카드 드랍 → 골드/카드 보상 (승리 직후·보스 분기 앞 — 호출 순서 = 판정 순서). **해금 3원화: `ECardUnlockType.EnemyDrop` (사용자 결정)** — 특정 조우 한정 드랍, 도감 발견 = 원장 재사용·`IsPoolEligible` 단일 관문 = 3경로 동시 차단·설계도 경로 자동 차단 (코드 0줄). **드랍 소유 = EnemyEncounterData (사용자 결정)** — 드랍 소유 단위 = 조우 통일, `EnemyData.dropTableId` 삭제. **카드 드랍 = 가중치 목록 개정 (사용자 결정)** — 단일 카드+확률 → 꽝 가중치 + `CardDropEntry` 목록 (DropTableData 미러, ⚠ 반영 대기). 잠정 규칙 8종·미결정 3건 = 완료 기록 참조. 임시 조치: 테스트 버튼 5종 (뷰 도입 시 제거). 잔여 소화: TownManager `testDiscoveryCard` 제거·테스트 버튼 region 이동 (개정 24 예고 이행). **실검증 = 아트 작업 시점 일괄 (사용자 결정 — 아트 적용 상태가 직관적).** **다음: 7-3 (적 12종 + 조우 테이블 — 리소스 작업) 또는 7-6 (보스 1개)** |
| 개정 24 | 2026-07-29 | **P2-M7 7-5 카드 해금 2종 완료 (프로젝트 반영 확인)** — `Save/CardUnlockService` (구획 파사드 전례 — 원장·조회만, 추첨 = 7-4 소관). **해금 원장 = Town 구획** `unlockedCardCodeNames` (필드 추가·버전 유지 — 발견형/제작형 공용 단일 목록, 해금 확정 = **즉시 저장** — 기획서 13-1 즉시 영구 계약·보관 전송 즉시 귀속 전례). **설계도 매핑 = `ItemData.unlockCard`** (설계도 자체가 해금 대상 식별 — Dead Cells식, 별도 매핑 구조 기각·OnValidate 타입 불일치 경고). **기본 해금 = `CardData.isDefaultUnlocked` + 조회 시 합집합** (저장 시딩 없음 — 마이그레이션 무관, 초기 풀 = 에셋 체크 코드 0줄). 잠정 규칙: 풀 제외 = 강화·저주 / 중복 = 무시 / **이미 해금 카드의 설계도 = 미소모 경고** / 소모 = 검사 후 일괄 차감 / 제작형의 발견 경로 해금 = 차단. 임시 조치: TownManager 테스트 2종 (서고 팝업·보상 굴림 도입 시 각각 제거). 사소 잔여: 테스트 버튼 region 밖 배치 → 임시 테스트 구획 이동. **실검증 = 아트 작업 시점 이월 (사용자 결정).** **다음: 7-4 로직 계층 (보상 굴림·골드 자원·구매/카드 제거 API — 해금 풀 추첨 실소비처) 또는 7-3 (적 12종 + 조우 테이블)** |
| 개정 23 | 2026-07-29 | **UI 작업 방침 개정 (사용자 결정)** — 신규 전용 뷰(보상 화면·상점 격자·마을 팝업·보관 선택 UI 등)는 **플레이스홀더 선작성 없이 아트 작업 시점에 스타일을 먼저 확정하고 그에 맞춰 뷰 코드 작업** (개정 20 마을 팝업 코드째 이월·6-3 보관 선택 UI 잔여 결정의 일반화 — 스타일 미확정 상태의 뷰 코드 = 재작업 리스크). 기존 플레이스홀더 뷰는 유지, 교체 시 유지 계약(Init/Release·구독+표시만 — 개정 17) 그대로. 뷰 뒤의 로직 계층(보상 굴림·골드 자원·구매/카드 제거 API)은 UI 없이 선행 가능. 7-4 잔여 재분류: 상점·보상 화면 뷰 = 아트 시점 / 로직 = 선행 가능 / 이벤트 10개 = 리소스 작업. **다음: 7-5 (카드 해금 2종 — 순수 로직/저장 작업, 보상·상점의 해금 풀 추첨 전제) → 7-4 로직 계층** |
| 개정 22 | 2026-07-29 | **P2-M7 7-2 유물 시스템 완료 (프로젝트 반영 확인) + 미배선 트리거 3종 발화 지점 연결 + Effect.Trigger 의존 정리** — `Data/RelicData` (**Data 흡수** — `Relic/` 폴더 신설 보류·기준 명문화(유물 고유 런타임 로직 등장 시). 등급/전용 캐릭터(null = 공용)/TriggerEffect 목록 = D4 패시브 구조 공유). **등급 = ERarityType 무수정 (사용자 결정 — Boss 신설·Unique 개명 모두 기각, 보스 유물 = 기존 `Unique(고유)` 값 매핑 — 기획 용어 ↔ 코드 분리 매핑).** 획득 관리 = DungeonState (`relics` **획득 순 = 발화 순 원본**·중복 무시 = 유일 보유 잠정) + 저장 편입 `relicCodeNames` (필드 추가·버전 유지·미등록 = 경고 스킵 — 아이템 전례) + 테스트 획득 버튼 (임시 조치 — 보상 화면(7-4 잔여) 시 제거). **발화 순서 계약: 파티 패시브(스폰 순) → 유물(획득 순).** 의존 정리 (개정 16 예고 이행): 소유자 ITargetable 전환 + HandleCardPlayed 제거 (조립 지점 어댑터 중계) = Effect.Trigger의 Battle·Card 참조 소멸. 파티 범위 등록 (공용 유물 시전자/대상 = 발화 시점 첫 생존자 / 전용 = 소유자 등록·사망 시 미발화·파티 부재 = 미등록 경고). 트리거 3종: TakeDamage = 파티 OnDamaged·RaiseFor(피격자·원본 > 0) / DealDamage = 적 OnDamaged + 카드 귀속 구간 (어그로 병행·귀속 밖 = 미발화) / BattleEnd = 승리 시 1회 (덱 리셋 이전 — SAN 이월 반영). **재진입 가드** — 발화 중 재발화 = 무시 (무한 연쇄 방지 잠정·연쇄 유물 요구 시 큐잉 재검토). 데이터 윈도우 유물 탭 증축. 사소 정리 소화: EEnemyType 오타·TargetResolver 태그·isSingleTarget 제거 (사소 잔여 5건은 완료 기록 참조). **실검증 = 아트 작업 시점 이월 (사용자 결정).** **다음: 7-4 잔여 (보상 화면·상점 — 유물/카드 획득 실경로) 또는 7-5 (카드 해금 2종 — 메타 저장 연동)** |
